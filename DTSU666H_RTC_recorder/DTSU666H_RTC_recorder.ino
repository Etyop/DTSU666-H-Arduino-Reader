...
float Ua, Ub, Uc, Ia, Ib, Ic, Pt, Pa, Pb, Pc, Qt, Qa, Qb, Qc, St, Sa, Sb, Sc, F;  // Write here the name of all the values you want to get from Modbus (read the readme to see full list)
...
void updateVariables() {      // This function stores the Modbus Data being read at the register address in the variable name
  ...
  Pt = readModbusData(2126);
  Pa = readModbusData(2128);
  Pb = readModbusData(2130);
  Pc = readModbusData(2132);
  Qt = readModbusData(2134);
  Qa = readModbusData(2136);
  Qb = readModbusData(2138);
  Qc = readModbusData(2140);
  St = readModbusData(2142);
  Sa = readModbusData(2144);
  Sb = readModbusData(2146);
  Sc = readModbusData(2148);
...
void initCsvOnSD() {                                                                 // This function creates the header line on the CSV file
  dataFile.println("Time;Ua;Ub;Uc;Ia;Ib;Ic;Pt;Pa;Pb;Pc;Qt;Qa;Qb;Qc;St;Sa;Sb;Sc;F");  // Remember to add your variables and check if you need ; or , (READ THE REAAAAADMEEEEE)
}
